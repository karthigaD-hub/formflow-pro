...
export default function UserForm() {
  ...
  const loadSection = async () => {
    try {
      ...
      if (responseRes.success && responseRes.data) {
        ...
        setIsSubmitted(responseRes.data.isSubmitted);
        const existingAnswers: Record<string, string | string[]> = {};
        responseRes.data.responses.forEach((r) => {
          existingAnswers[r.questionId] = r.value;
        });
  ...
  const handleSave = async (silent = false) => {
    ...
    setIsSaving(true);
    try {
      const responses: QuestionResponse[] = Object.entries(answers).map(([questionId, value]) => ({
        questionId,
        value,
      }));
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {isSubmitted && <span className="flex items-center text-sm text-success"><CheckCircle className="h-4 w-4 mr-1" /> Submitted</span>}
          </div>
